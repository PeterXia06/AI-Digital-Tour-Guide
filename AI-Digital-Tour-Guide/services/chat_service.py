...
# 【修复点】复用 knowledge_service 的 ChromaDB 单例，消除重复内存占用
...
# 【修复点】retriever 已从 knowledge_service 单例导入，不再重复创建 ChromaDB 实例
...
def _update_stats(db: Session, session_id: str, emotion: str):
    ...
    【修复点】简化 is_first_today 判断逻辑：
    直接用 COUNT 查询当日对话数，当前对话已写入 DB，
    所以 count == 1 意味着只有当前这一条 = 今日首次。
    """
    today = date.today()

    # 统计该 session 今日对话总数（含当前已写入的记录）
